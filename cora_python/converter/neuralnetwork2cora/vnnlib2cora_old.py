...
def vnnlib2cora(file_path: str) -> Tuple[List[Interval], Specification]:
    ...
    if not Y:
        ...
        # Single constraint → create safeSet (matches MATLAB)
        spec = Specification(Y[0], 'safeSet')
    else:
        # Multiple constraints → convert to union of unsafe sets (matches MATLAB)
        # Y = safeSet2unsafeSet(Y)  # TODO: Implement proper conversion
        # For now, create unsafeSet with first polytope
        spec = Specification(Y[0], 'unsafeSet')
        
        # TODO: Add support for multiple unsafe sets
