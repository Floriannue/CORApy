"""
times - overloaded '.*' operator for reachSet objects (element-wise multiplication)
...
def times(R1, R2):
    """
    Overloaded '.*' operator for reachSet objects (element-wise multiplication)
    ...
    # If R2 is numeric, multiply all sets element-wise
    if isinstance(R2, (int, float, np.ndarray)):
        # Create new reachSet with scaled sets
        new_timePoint = {}
        new_timeInterval = {}
        
        # Scale time-point sets
    ...
    # If R2 is another reachSet, this is more complex (not commonly used)
        ...
        raise NotImplementedError("Element-wise multiplication between reachSet objects not implemented")
