...
def project(zB: 'ZonoBundle', dims: Union[List[int], Sequence[int]]) -> 'ZonoBundle':
    """
    Projects a zonotope bundle onto the specified dimensions.
    """
    # project for each zonotope
    for i in range(zB.parallelSets):
        zB.Z[i] = zB.Z[i].project(list(dims))
