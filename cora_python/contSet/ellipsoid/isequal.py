...
def isequal(E: 'Ellipsoid', S: Union['ContSet', np.ndarray, float, int], tol: Optional[float] = None) -> bool:
    logger.debug(f"isequal: E={E}, S={S}, tol={tol}")
    ...
    logger.debug(f"isequal: After tol default, tol={tol}")

    # Check input arguments
    logger.debug(f"isequal: Calling inputArgsCheck with E={E}, S={S}, tol={tol}")
    inputArgsCheck([
        [E, 'att', ['ellipsoid', 'numeric'], 'scalar'],
        [S, 'att', ['ContSet', 'numeric'], 'scalar'],
        [tol, 'att', 'numeric', ['nonnan', 'nonnegative', 'scalar']]
    ])
    logger.debug(f"isequal: inputArgsCheck passed.")

    # ensure that numeric is second input argument
    E, S = reorder_numeric(E, S)
    logger.debug(f"isequal: After reorder_numeric, E={E}, S={S}")
    
    # call function with lower precedence
    if hasattr(S, 'precedence') and isinstance(S, (E.__class__, S.__class__)) and S.precedence < E.precedence:
        logger.debug(f"isequal: Delegating to S.isequal due to precedence. S={S}, E={E}")
    ...
    if not equal_dim_check(E, S, True):
        ...
        logger.debug(f"isequal: Dimensions do not match. E.dim()={E.dim()}, S.dim()={getattr(S, 'dim', lambda: S.size if isinstance(S, np.ndarray) and S.ndim == 0 else S.shape[0])()}")
        return False
    ...
    if isinstance(S, Ellipsoid):
        logger.debug(f"isequal: S is an Ellipsoid. Calling _aux_isequal_ellipsoid.")
        res = _aux_isequal_ellipsoid(E, S, tol)
        return res
    
    # If S is numeric (handled by reorder_numeric to be second arg)
    if isinstance(S, (int, float, np.ndarray)):
        logger.debug(f"isequal: S is numeric. E.dim()={E.dim()}, S={S}")
        if E.dim() == 1 and isinstance(S, (int, float)): # 1D ellipsoid vs scalar
            logger.debug(f"isequal: 1D Ellipsoid vs scalar numeric.")
            ...
            return E.representsa_('point', tol) and np.allclose(E.q, np.array([[S]]), atol=tol)
             ...
             logger.debug(f"isequal: Ellipsoid vs 2D row/column vector (point representation).")
             return E.representsa_('point', tol) and np.allclose(E.q, S.reshape(-1,1), atol=tol)
        else:
             logger.debug(f"isequal: Generic numeric input not handled for point comparison. Raising CORA:noops.")
             raise CORAerror('CORA:noops', E, S)


    logger.debug(f"isequal: S is an unhandled ContSet type. Raising CORA:noops.")
    raise CORAerror('CORA:noops', E, S)

def _aux_isequal_ellipsoid(E: 'Ellipsoid', S: 'Ellipsoid', tol: float) -> bool:
    logger.debug(f"_aux_isequal_ellipsoid: E={E}, S={S}, tol={tol}")
    res = False

    if E.dim() != S.dim():
        logger.debug(f"_aux_isequal_ellipsoid: Dimension mismatch. E.dim()={E.dim()}, S.dim()={S.dim()}")
        return res

    E_empty = E.representsa_('emptySet', np.finfo(float).eps)
    S_empty = S.representsa_('emptySet', np.finfo(float).eps)
    logger.debug(f"_aux_isequal_ellipsoid: E_empty={E_empty}, S_empty={S_empty}")

    if (E_empty and not S_empty) or (not E_empty and S_empty):
        logger.debug(f"_aux_isequal_ellipsoid: One empty, one not. Returning False.")
        return False
    elif E_empty and S_empty:
        logger.debug(f"_aux_isequal_ellipsoid: Both empty. Returning True.")
        return True

    res = np.all(withinTol(E.Q, S.Q, tol)) and np.all(withinTol(E.q, S.q, tol))
    logger.debug(f"_aux_isequal_ellipsoid: Numerical comparison result: {res}")
