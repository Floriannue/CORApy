...
def aux_lb_sec2_ft_IN_kg(prev_val):
    """
    Convert lb*sec^2/ft to kg
...
def aux_lb_ft_sec2_IN_kg_m2(prev_val):
    """
    Convert lb*ft*sec^2 to kg*m^2
...
def aux_rad_ft_lb_IN_rad_sec2_kg_m2(prev_val):
    """
    Convert rad/(ft*lb) to rad/(N*m) = rad*s^2/(kg*m^2)
...
def aux_ft2_IN_m2(prev_val):
    ...
    1ft is 0.3048 m
    """
    return 0.3048**2 * prev_val


def aux_lbs_ft_IN_N_m(prev_val):
    """
    Convert lbs/ft to N/m
...
def aux_lb_sec_ft_IN_N_s_m(prev_val):
    """
    Convert lb*sec/ft to N*sec/m
...
def aux_ft_lb_rad_IN_N_m_rad(prev_val):
    """
    Convert lb*ft/rad to N*m/rad
...
def aux_ft_lb_IN_m_N(prev_val):
    """
    Convert ft/lb to m/N
...
def aux_rad_ft_IN_rad_m(prev_val):
    """
    Convert rad/ft to rad/m
...
def highorderBicycleDynamics(x, u):
    """
    highorderBicycleDynamics - generates bicycle model for comparison with 
                               the DOT (department of transportation) car 
                               dynamics
    ...
    Args:
        x: state vector (18-dimensional)
        u: input vector (2-dimensional: steering angle and acceleration)
        
    Returns:
        f: time-derivative of the system state
    """
    # Parameter structure (mirroring MATLAB struct p)
    class P:
        pass
    ...
    p.I_y_w = None  # wheel inertia, to be determined [kg m^2]
    ...
    # Create nested structure for tire parameters
    class Tire:
        pass
    ...
    # model
    f = np.zeros((18, 1))
    f[0, 0] = (mu / (x[3, 0]**2 * (lr + lf)) * (C_Sr * (g * lf + (x[7, 0] + u[1, 0]) * h) * lr - C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h) * lf) - 1) * x[2, 0] \
        - mu / (x[3, 0] * (lr + lf)) * (C_Sr * (g * lf + (x[7, 0] + u[1, 0]) * h) + C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h)) * x[0, 0] \
        + mu / (x[3, 0] * (lr + lf)) * (C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h)) * (x[6, 0] + u[0, 0])
    f[1, 0] = x[2, 0]
    f[2, 0] = -mu * m / (x[3, 0] * I * (lr + lf)) * (lf**2 * C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h) + lr**2 * C_Sr * (g * lf + (x[7, 0] + u[1, 0]) * h)) * x[2, 0] \
        + mu * m / (I * (lr + lf)) * (lr * C_Sr * (g * lf + (x[7, 0] + u[1, 0]) * h) - lf * C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h)) * x[0, 0] \
        + mu * m / (I * (lr + lf)) * lf * C_Sf * (g * lr - (x[7, 0] + u[1, 0]) * h) * (x[6, 0] + u[0, 0])
    f[3, 0] = (x[7, 0] + u[1, 0])
    f[4, 0] = x[3, 0] * np.cos(x[0, 0] + x[1, 0])
    f[5, 0] = x[3, 0] * np.sin(x[0, 0] + x[1, 0])
    f[6, 0] = 0
    f[7, 0] = 0
    f[8, 0] = 0
    f[9, 0] = 0
    ...
    # system dynamics (commented out in MATLAB)
