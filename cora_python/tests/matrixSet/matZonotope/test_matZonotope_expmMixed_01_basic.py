"""
Test file for expmMixed - translated from MATLAB

This test verifies the expmMixed function for matZonotope.
...
# MATLAB I/O pairs from debug_matlab_expmMixed.m
tol = 1e-10


class TestExpmMixed:
    """Test cases for expmMixed"""
    
    def test_expmMixed_01_simple_2x2(self):
        ...
        eI_center_expected = np.array([[0.1170, -0.0863], [0.0820, 0.1593]])
        np.testing.assert_allclose(eI.int.center(), eI_center_expected, rtol=tol, atol=tol)
        
        # eI radius (from MATLAB):
        eI_rad_expected = np.array([[0.0981, 0.0920], [0.0913, 0.1263]])
    ...
    def test_expmMixed_02_3x3(self):
        ...
        eZ2_center_expected = np.array([[0.5025, 0.7500, 0], [-0.7500, 0.1275, 0], [0, 0, 0.5025]])
        np.testing.assert_allclose(eZ2.C, eZ2_center_expected, rtol=tol, atol=tol)
        
        # eI2 center (from MATLAB):
        eI2_center_expected = np.array([[0.0833, -0.1242, 0], [0.1208, 0.1472, 0], [0, 0, -0.1709]])
