"""
test_verifyFast_heat3D_HEAT01 - heat3d benchmark from
    the 2023 ARCH competition

Syntax:
    text = test_verifyFast_heat3D_HEAT01()
...
class TestVerifyFastHeat3DHEAT01:
    """Test class for heat3D HEAT01 benchmark verification"""
    
    def test_verifyFast_heat3D_HEAT01(self):
        """Test heat3D benchmark HEAT01 from ARCH 2023 competition"""
        # System Dynamics ---------------------------------------------------------
        
        # load system matrices
        # MATLAB: load('heat5');
        from cora_python.g.macros.CORAROOT import CORAROOT
        mat_file_path = os.path.join(CORAROOT(), 'models', 'Cora', 'heat3D', 'heat5.mat')
        ...
        # MATLAB: indMid = ceil((size(A,1))/2);
        indMid = int(np.ceil((A.shape[0]) / 2))
        ...
        # MATLAB: text = ['Heat3D,HEAT01,',num2str(res),',',num2str(savedata.tComp)];
        text = f'Heat3D,HEAT01,{res},{savedata.get("tComp", "N/A")}'
        
        return text


def test_verifyFast_heat3D_HEAT01():
    """Test function for heat3D HEAT01 benchmark verification.
    
    Runs all test methods to verify correct implementation.
    """
    test = TestVerifyFastHeat3DHEAT01()
    result = test.test_verifyFast_heat3D_HEAT01()
    
    print(f"test_verifyFast_heat3D_HEAT01: result = {result}")
    return result


if __name__ == "__main__":
    test_verifyFast_heat3D_HEAT01()
