...
Syntax:
    pytest cora_python/tests/contDynamics/nonlinearSys/testLong_nonlinearSys_evalNthTensor.py
...
class TestLongNonlinearSysEvalNthTensor:
    """Test class for evalNthTensor functionality (long test with N=10)"""
    
    def test_long_nonlinearSys_evalNthTensor(self):
        """Test tensor generation and evaluation for 2D example with N=10 test points"""
        # 2D-example --------------------------------------------------------------
        # MATLAB: syms x y
        ...
        # MATLAB: for i = 1:N
        # MATLAB:     assertLoop(abs(res_test(i)-res_real(i)) <= 1e-14,i)
        # MATLAB: end
        for i in range(N):
            diff = abs(res_test[i] - res_real[i])
            assert diff <= 1e-14, \
                f"Tensor evaluation mismatch at index {i}: res_test={res_test[i]}, res_real={res_real[i]}, diff={diff}"


def testLong_nonlinearSys_evalNthTensor():
    """Test function for evalNthTensor method (long test with N=10).
    
    Runs all test methods to verify correct implementation.
    """
    test = TestLongNonlinearSysEvalNthTensor()
    test.test_long_nonlinearSys_evalNthTensor()
    
    print("testLong_nonlinearSys_evalNthTensor: all tests passed")
    return True
