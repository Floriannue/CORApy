...
def testLong_zonotope_reduce():
    """
    Unit test function of reduce
    """
    # tolerance
    tol = 1e-6
    
    # create zonotope
    c = np.array([[-1.3], [2.9], [2.0], [3.2]])  # Column vector
    G = np.array([
    ...
    Zorig = Zonotope(c, G)
    Zorig_2D = Zonotope(c[:2], G[:2, :10])
    
    # Test Girard - order 1
    Zred = Zorig.reduce('girard', 1)
    ...
    # check result
    Z_true = Zonotope(Zmat_true[:, 0:1], Zmat_true[:, 1:])
    assert Zred.isequal(Z_true, tol)
    
    # Test Girard - order 3
    Zred = Zorig.reduce('girard', 3)
    ...
    # Test Combastel - order 1
    Zred = Zorig.reduce('combastel', 1)
    ...
    # Test Combastel - order 3
    Zred = Zorig.reduce('combastel', 3)
    ...
    # Test MethC - order 1
    Zred = Zorig.reduce('methC', 1, [12, 6])
    ...
    # Test MethC - order 3
    Zred = Zorig.reduce('methC', 3, [12, 6])
    ...
    # Test redistribute - order 1
    Zred = Zorig.reduce('redistribute', 1)
    ...
    # Test redistribute - order 3
    Zred = Zorig.reduce('redistribute', 3)
