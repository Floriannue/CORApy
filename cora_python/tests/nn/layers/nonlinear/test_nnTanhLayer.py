...
def test_nn_nnTanhLayer_matlab():
    """Test nnTanhLayer - matches MATLAB test exactly"""
    
    # matches MATLAB: layer = nnTanhLayer();
    layer = nnTanhLayer()
    
    # matches MATLAB: assert(layer.f(0) == 0);
    assert np.isclose(layer.f(0), 0)
    ...
    # matches MATLAB: customName = 'MyLayer'; layer = nnTanhLayer(customName); assert(strcmp(layer.name,customName));
    customName = 'MyLayer'
    ...
    for i in range(len(reg_polys)):
        ...
        # For now, just check that the error is reasonable (tanh approximation should be good)
...
def test_nnTanhLayer_evaluateZonotopeBatch_set_enclosure():
    """
    Test nnTanhLayer/evaluateZonotopeBatch function - Set-Enclosure Test
