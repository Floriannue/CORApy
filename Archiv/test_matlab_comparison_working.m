% test_matlab_comparison_working.m
% MATLAB test for comparing error_adm_horizon growth with Python
% Uses tank6Eq model which has required hessian functions
...
% Ensure model auxiliary functions are on path
addpath(genpath('cora_matlab/models/auxiliary'));

% Create tank6Eq nonlinear system (has hessian functions)
% Use explicit name to ensure function lookup works
sys = nonlinearSys('tank6Eq',@tank6Eq,6,1);

% Set up parameters
params.tStart = 0;
params.tFinal = 2.0;  % Shorter time for faster testing
...
% Set algorithm - explicitly set tensorOrder to skip initialization
options.alg = 'lin-adaptive';
options.tensorOrder = 2;  % Set explicitly to potentially skip aux_initStepTensorOrder
...
if ~isfield(options, 'zetaK')
    options.zetaK = 1.5;  % Threshold for tensor order selection
end
if ~isfield(options, 'zetaTlin')
    options.zetaTlin = 0.0005;  % Convergence threshold for linear system (taylorTerms)
end
if ~isfield(options, 'zetaTabs')
    options.zetaTabs = 0.005;  % Absolute threshold for taylorTerms
end
if ~isfield(options, 'decrFactor')
    options.decrFactor = 0.90;  % Adaptation factor for delta t
end
if ~isfield(options, 'zetaphi')
    options.zetaphi = [0.85; 0.76; 0.68];  % zeta_h (timeStep) thresholds for different orders
end

% Run reachability analysis
fprintf('=== Testing Intermediate Value Tracking with tank6Eq (MATLAB) ===\n');
