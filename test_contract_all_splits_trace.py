"""Trace through 'all' contractor with splits=2 step by step"""
import numpy as np
...
print("=== Tracing 'all' contractor with splits=2 ===")
print(f"Initial domain: {dom}")

# Simulate the exact splitting logic
list_intervals = [dom]
...
for i in range(2):  # splits = 2
    ...
    for j in range(len(list_intervals)):
        ...
        # Check if it's a point interval
        width = list_intervals[j].sup - list_intervals[j].inf
        ...
        for k in range(len(domSplit)):
            ...
            print(f"      Contracting with 'all' algorithm (iter=2, splits=None)...")
