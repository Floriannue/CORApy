"""Debug the constraint loop in _aux_contains_P_Hpoly"""
import numpy as np
...
# Simulate the exact code path in _aux_contains_P_Hpoly
combined_A, combined_b = priv_equality_to_inequality(P1.A, P1.b, P1.Ae, P1.be)
n_dim = P1.dim()
...
print("After normalization and compaction:")
...
for i in range(combined_b.shape[0]):
    ...
    b_set_sup_val, _, _ = supportFunc_(Z2, normal_vector, 'upper')
    ...
    print(f"  withinTol(b_polytope, b_set_sup_val, tol): {withinTol(b_polytope, b_set_sup_val, tol)}")
    
    condition = b_set_sup_val > b_polytope and not withinTol(b_polytope, b_set_sup_val, tol)
    print(f"  Violation condition: {condition}")
    
    if condition:
        print(f"  *** VIOLATION DETECTED ***")
        res = False
        if not scalingToggle:
            print(f"  Early return (scalingToggle=False)")
            break
        else:
            scaling_current = b_set_sup_val / b_polytope
            print(f"  scaling_current: {scaling_current}")
